...
```{r bar}
df %>%
  ggplot(aes(x = Was_a_Search_Conducted)) +
  geom_bar()
```
...
**The data is imbalanced. There are much fewer Yes results than No.**
...
**Answer: It appears that there are some ages that are occur more often than ages next to them (e.g., 20, 25, 30, 35, 40). This seems to suggest that sometimes these ages are rounded as we would have no reason to suspect the data truly would be larger just for every five years.**
...
```{r violin}
ggplot(df, aes(x = Was_a_Search_Conducted, y = Driver_Age, fill = Was_a_Search_Conducted)) +
  geom_violin()
```
...
**Answer: It appears that the searches were conducted on a slightly younger population than the population that was not searched. But other answers could include no relationship, need better plot to make a judgment.**
...
**Answer: The long term trends is flat but there were short term fluctuations (e.g., dropped in late 2016 but then increased in 2017)**
https://dsba5122fall2019.slack.com/archives/CLUCHHQPJ/p1569273552006200
**Plot** the same plot but add in `facet_wrap()` by the `Reason_for_Stop` variable.

```{r stops-by-reason}
count(df, Date, Reason_for_Stop) %>%
  ggplot(aes(x = Date, y = n)) +
  geom_point() +
  geom_line() +
...
**Answer: Since several of the variables are very small, they appear like flat lines given the same y-axis scale for the facets (multiple graphs).**
...
count(df, Date, Reason_for_Stop) %>%
  ...
  geom_point() +
  geom_line() +
  facet_wrap(~Reason_for_Stop, scale = "free_y")
```

Which type of police stop has had the most volatility (i.e., big swings in number of stops)? xxxxx

**Answer: Answers can vary. True answer is that it's too difficult to assess with different label. True answer would be Vehicle Regulatory since it's the largest category.**

What is one problem with allowing the y-axis be free? xxxxx

**Answer: it's difficult to compare horizontally (across) plots. For example, Speeding is on a different scale than SeatBelts**
...
g <- cmpd %>%
  mutate(Name = as.character(DNAME)) %>%
  mutate(Name = stringr::str_replace_all(Name, " Division","")) %>%
  ggplot() +
...
```{r albers}
g + coord_sf(crs = st_crs(102003)) # Albers projection
```


Do you notice any difference between the two maps? xxxxx

**Answer: not much. The plots shifted some but these changes typically have a larger impact, i.e., show up on a country or continent level, not a county/city level**
