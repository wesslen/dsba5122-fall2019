...
Modify the object in the YAML parameter `output:` (e.g., html_document, pdf_document, word_document.)
...
`ggplot2`, `tibble`, `tidyr`, `readr`, `purrr`, `dplyr`, `stringr`, and `forcats` (older versions of `tidyverse` may only read some of these)
...
704 rows (observations) and 5 columns (variables)
...
Open to interpretations so almost any three responses are good (so long as they're thought out). Some ideas can include (only need 3 of these): 

1. Need better x/y/title labels

2. Poor color scheme/background color

3. Could highlight individual labels (e.g., point out which country is which)

4. Could add in a "trend/regression" line

5. Modify the x and/or y scales (e.g., start y axis at 0 and go to 1, etc.)
...
1. Remove the `eval=T` parameter from each chunk to run each chunk when `knit` your output.
...
Install `cowplot` and `colorspace` and call these libraries. Also, remove the `eval=T` parameter from each chunk to run each chunk when `knit` your output. (hint: you can do this for all parts via Edit > Replace and Find or CTRL + F)

```{r libraries,eval=T,warning=F,message=F}
...
`warning=F` : If FALSE, knitr will not display any warning messages generated by the code.

`message=F` : If FALSE, knitr will not display any messages generated by the code
...
`cowplot::` directly calls the function from the package `cowplot`. The `::` leaves calling the library (`library(cowplot)`) as optional. Since we previously called `library(cowplot)`, it's not necessary but okay to plot to explicitly show where the function `theme_minimal_hgrid` is from the `cowplot` package. It would be necessary if we didn't call `cowplot` library explicitly and if we thought we'd only need to use one function from this library.
...
```{r modify-color,eval=T, message = FALSE}
# Okabe Ito colors
...
`y~x` assumes a standard linear regression, i.e., the change between a unit of the x variable is the same for all values of x. This would prevent any curvature in our regression line. (Any answer close to this is okay)
...
The `%+%` will replace the underlying dataframe in the ggplot object `g`. This necessary since we modified the underlying dataframe `corrupt` to add in the new column `label`, i.e., if we didn't replace the dataframe, the ggplot would not find the new column `label` we created.
...
For a given level of perceived corruption (x value), countries above the regression line have a *higher* HDI than expected for its level of perceived corruption. Alternatively, countries below the regression line have a *lower* HDI than expected for its level of perceived corruption. (Something similar is okay)
...
Open to many answers. So long as it's thoughtful, it's okay.
